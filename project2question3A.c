...
static int q = 0;
static int top = 0;
void *free_pool[100];// Datastructure to hold freeMemory to hold
...
start = clock();        
        for (i = 0; i < k; i++)
        {
            if (used > (5242880)){//10485760 * 0.5   // When only 50% of  10MB is available
                do{
                    //printf("WAITING for memory to be released\n");
                }while (used > 5242880);
                while ((p[i].mem * 1024) < used){// Waiting for process required memory
                    //printf("WAITING for required memory\n");
                }
                    p[i].mem_ptr = my_malloc(p[i].mem * 1024, 0);// Allocating memory from free pool
                //}while ((p[i].mem * 1024) <= ((5242880) - used));
            } else {
                   printf("Allocated %d\n", i);
